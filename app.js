...
function chartSvg(data){
  ...
  var axisLabels=[];
  for(var i=0;i<5;i++){
    axisLabels.push((max-range*(i/4)).toFixed(2));
  }
  var gridLines=[40,80,120,160,200].map(function(y){return'<line x1="0" y1="'+y+'" x2="'+w+'" y2="'+y+'"/>'}).join('');
  return '<svg viewBox="0 0 '+w+' '+(h+20)+'" preserveAspectRatio="none" style="width:100%;height:100%"><defs><linearGradient id="fill" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stop-color="#a9ef38" stop-opacity=".18"/><stop offset="1" stop-color="#a9ef38" stop-opacity="0"/></linearGradient></defs><g class="grid">'+gridLines+'</g><path class="area" d="'+area+'" fill="url(#fill)"/><path class="line" d="'+path+'" fill="none" stroke="#a9ef38" stroke-width="2"/></svg>';
}

let chartHistory=[];

async function fetchMarkets(){
  try{const r=await fetch('https://api.binance.com/api/v3/ticker/24hr?symbol=BTCUSDT');const t=await r.json();const p=parseFloat(t.lastPrice);const c=parseFloat(t.priceChangePercent);markets[3][4].push(p);if(markets[3][4].length>30)markets[3][4].shift();markets[3]=['BTC/USDT',p.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2}),(c>=0?'+':'')+c.toFixed(2)+'%',c>=0?'up':'down',markets[3][4]]}catch(e){}
  try{const r=await fetch('https://api.binance.com/api/v3/ticker/24hr?symbol=PAXGUSDT');const t=await r.json();const p=parseFloat(t.lastPrice);const c=parseFloat(t.priceChangePercent);markets[2][4].push(p);if(markets[2][4].length>30)markets[2][4].shift();markets[2]=['XAU/USD',p.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2}),(c>=0?'+':'')+c.toFixed(2)+'%',c>=0?'up':'down',markets[2][4]]}catch(e){}
  try{const r=await fetch('https://api.exchangerate-api.com/v4/latest/USD');const j=await r.json();if(j&&j.rates){
    var usdTRY=j.rates.TRY;
    var eurTRY=j.rates.TRY/j.rates.EUR;
...
function updateMarketUI(){
  ...
  markets.forEach(function(m,i){
    var dir=m[3];
    var changeClass=dir==='up'?'up':'down';
    var changeIcon=dir==='up'?'&#9650;':'&#9660;';
    var sparkSvg=sparkLine(m[4],colors[i]);
    html+='<div class="market"><div><small>'+m[0]+'</small><strong>'+m[1]+'</strong></div><span class="change '+changeClass+'">'+m[2]+'</span><div class="spark">'+sparkSvg+'</div></div>';
  });
...
function render(){
  ...
  var icons=['\u2301','\u25D2','\u300A'];
  var gradients=['linear-gradient(135deg,#1a1040,#0d0a1a 60%)','linear-gradient(135deg,#0a1628,#060d18 60%)','linear-gradient(135deg,#1a0d2e,#0d0618 60%)'];
  var glows=['radial-gradient(circle at 50% 40%,#7c4aed40,transparent 70%)','radial-gradient(circle at 50% 40%,#3b82f640,transparent 70%)','radial-gradient(circle at 50% 40%,#a855f740,transparent 70%)'];
  return '<article class="project '+(x.featured?'featured':'')+'">'+
    '<div class="project-img" style="background:'+gradients[i]+';position:relative;overflow:hidden">'+
      '<div style="position:absolute;inset:0;background:'+glows[i]+'"></div>'+
      '<span class="project-icon" style="position:relative;z-index:1">'+(icons[i]||'\u2726')+'</span>'+
    '</div>'+
    '<div class="project-no">0'+(i+1)+'</div>'+
    '<button class="project-add" aria-label="Incele">\u271A</button>'+
    '<h3>'+esc(t(x.name,l))+'</h3>'+
    '<p>'+esc(t({tr:x.tr,en:x.en},l))+'</p>'+
    '<div class="tags">'+(x.tech||[]).slice(0,4).map(function(z){return'<span>'+esc(z)+'</span>'}).join('')+'</div>'+
    '<a href="#contact">PROJEY\u0130 \u0130NCELE&nbsp; \u2192</a>'+
  '</article>';
}).join('')+'</div><div id="skills" class="tech-strip">'+(d.skills||[]).slice(0,8).map(function(x){return'<span>'+esc(x)+'</span>'}).join('')+'</div></div></section><section id="market" class="bottom-grid wrap reveal"><div class="market-chart"><div class="panel-title"><span>P\u0130YASA TAK\u0130P PANEL\u0130</span><b>'+(markets[0][1]!=='--'?markets[0][1]:'---')+'</b></div><div class="chart-wrap"></div><div class="periods"><b>1G</b><span>1H</span><span>1A</span><span>3A</span><span>1Y</span><span>T\u00dcM\u00dc</span></div></div><div id="about" class="about-card"><div><span class="kicker">HAKKIMDA</span>'+about.slice(0,2).map(function(x){return'<p>'+esc(x)+'</p>'}).join('')+'<div class="pills"><span>Problem \u00c7\u00f6zme</span><span>Yenilik\u00e7i</span><span>Disiplinli</span><span>\u00d6\u011frenmeye A\u00e7\u0131k</span></div><a href="#about">DAHA FAZLA&nbsp; \u2192</a></div><div class="about-photo"><img src="'+esc(p.photo||'photo.jpg')+'" alt="'+esc(p.name)+'"><div class="about-meta"><span>\u260E Istanbul, T\u00fcrkiye</span><span>\u2316 16 Ya\u015f\u0131nda</span></div></div></div></section><section id="journey" class="sr-only"></section><section id="contact" class="contact"><div class="footer-grid wrap"><div><span class="dot"></span>\u015eU AN<br><small>\u00c7al\u0131\u015f\u0131yorum...</small></div><div class="code-footer"><b>const</b> hedef = "Daha iyi bir yar\u0131n";<br><b>while</b>(\u00fcretiyorum) {<br>&nbsp;&nbsp;&nbsp;&nbsp;fark_yarat();<br>}</div><div>\u2316 &nbsp; KONUM<br><small>\u0130stanbul, T\u00fcrkiye</small></div><div>\u2709 &nbsp; E-POSTA<br><small>'+esc(p.email)+'</small></div><div>SOSYAL<br><small><a href="https://github.com/'+esc(p.github)+'" target="_blank" style="color:var(--lime)">&#9673;</a> &nbsp; <a href="#" target="_blank" style="color:var(--lime)">&#9741;</a> &nbsp; <a href="#" target="_blank" style="color:var(--lime)">&#9673;</a></small></div></div></section></main><footer>\u00a9 '+new Date().getFullYear()+' '+esc(p.name)+'. T\u00fcm haklar\u0131 sakl\u0131d\u0131r.</footer>';
